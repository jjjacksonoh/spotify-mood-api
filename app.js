...
var routes = require('./src/routesNew');
...
app.get('/login', function(req, res) {
  ...
  var scope = 'user-read-private user-read-email user-read-recently-played user-top-read user-read-playback-position';
    ...
    request.post(authOptions, function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        request.get(tracks, function(error, response, body) {
          //console.log(body)
          for(i=0; i < body.items.length; i++) {
            topTracks.push(body.items[i].name)
          }
...
const showTopEverything = () => {
  document.getElementById('topEverything').innerHTML = `
  <li>${topArtists[0]}</li>
  <li>${topTracks[0]}</li>`
}